...
| HTML-formatted label (e.g. `value="<font color=\"#0070F2\">Title</font>"`) looks fine in a script but the `.drawio` fails to parse | Escaping only the inner text and leaving the wrapper's own `<font>`/`<b>` tags and their `"` attribute quotes un-escaped produces invalid XML (unterminated attribute). Build the full HTML string first (tags + text, with plain `<`/`>`/`"`), then run it through **one** full escape pass (`&`→`&amp;`, `<`→`&lt;`, `>`→`&gt;`, `"`→`&quot;`) before inserting into the `value="..."` attribute — matching how draw.io itself stores these labels (e.g. `value="&lt;b style=&quot;color:#0070F2;&quot;&gt;Title&lt;/b&gt;"`). `get_errors` does not reliably catch this for `.drawio` files — verify with a real XML parser (e.g. Python `xml.etree.ElementTree.fromstring`) after generating. |
